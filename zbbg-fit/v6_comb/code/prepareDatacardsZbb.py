...
# def createCardTemplateCombined(cat_score,f_ws,sys={},ws_name="w"):
#     """Crea datacard con modello combinato per zjet+wjet+gj"""
    
#     dc_name = "datacard_zbb_combined_"+cat_score+".txt"
...
#     f.write("kmax * \n")
#     f.write("------------ \n")
#     f.write("shapes combined "+f_ws+" "+ws_name+":"+"pdf_combined\n")
#     f.write("shapes zg "+f_ws+" "+ws_name+":"+"pdf_zg\n")
#     f.write("shapes data_obs "+f_ws+" "+ws_name+":"+"data\n")
#     f.write("------------ \n")
#     f.write("bin   \n")
#     f.write("observation -1 \n")        
#     f.write("------------ \n")
#     f.write("process   combined  zg \n")
#     f.write("process   0         1 \n")
#     f.write("rate   1         1 \n")
#     f.write("------------ \n")
#     f.write("lumi_13p6TeV  lnN  1.014  1.014\n")
#     f.write("combined_scale  lnN  0.95/1.05  -\n")
#     if "NLOEWK_corr_combined" in sys:
#         f.write("NLOEWK_corr_combined  lnN  %.2f  -\n"%(sys["NLOEWK_corr_combined"]))
#     else:
#         f.write("NLOEWK_corr_combined  lnN  0.93/1.08  -\n")
#     if "pdf_combined" in sys:
#         f.write("pdf_combined lnN  %.2f  -\n"%(sys["pdf_combined"]))
#     else:
#         f.write("pdf_combined lnN  0.98/1.02  -\n")
#     if "NLOEWK_corr_zg" in sys:
#         f.write("NLOEWK_corr_zg  lnN  -  %.2f\n"%(sys["NLOEWK_corr_zg"]))
#     else:        
#         f.write("NLOEWK_corr_zg  lnN  -  0.92/1.09\n")
...
#     else:
#         f.write("pdf_zg lnN  -  0.98/1.02\n")
#     f.write("CMS_scale_j  lnN  0.97/1.03  0.97/1.03\n")
#     f.write("CMS_res_j  lnN  0.98/1.01  0.98/1.01\n")
#     f.write("CMS_trigger_zbb  lnN  0.98/1.02  0.98/1.02\n")
...
#########################################
## Fit combinato di tutti i processi  ##
#########################################

# Istogramma combinato sommando zjet + wjet + gj
h_combined = h_zjet.Clone("h_combined")
h_combined.Add(h_wjet)
h_combined.Add(h_gj)

rh_combined = ROOT.RooDataHist("combined","",obs_list,h_combined)

# Modello unificato con somma di gaussiane per tutti i processi
# Prima gaussiana 
combined_mean1 = ROOT.RooRealVar("combined_mean1","combined_mean1",85.0,70,110)
combined_sigma1 = ROOT.RooRealVar("combined_sigma1","combined_sigma1",8.0,2,25)
pdf_combined_gauss1 = ROOT.RooGaussian("pdf_combined_gauss1","pdf_combined_gauss1",obs,combined_mean1,combined_sigma1)

# Seconda gaussiana
combined_mean2 = ROOT.RooRealVar("combined_mean2","combined_mean2",85.0,70,110)
combined_sigma2 = ROOT.RooRealVar("combined_sigma2","combined_sigma2",20.0,10,60)
pdf_combined_gauss2 = ROOT.RooGaussian("pdf_combined_gauss2","pdf_combined_gauss2",obs,combined_mean2,combined_sigma2)

combined_gauss_frac = ROOT.RooRealVar("combined_gauss_frac","combined_gauss_frac",0.6,0.,1.)

# Modello finale combinato
pdf_combined = ROOT.RooAddPdf("pdf_combined","pdf_combined",ROOT.RooArgList(pdf_combined_gauss1,pdf_combined_gauss2),ROOT.RooArgList(combined_gauss_frac),True)
fit_combined_res = pdf_combined.fitTo(rh_combined,ROOT.RooFit.Save(),ROOT.RooFit.Optimize(1),ROOT.RooFit.SumW2Error(True),ROOT.RooFit.Minimizer("Minuit2"))

h_fit_combined = pdf_combined.createHistogram("h_fit_combined",obs,ROOT.RooFit.Binning(obs.getBins()*10))
h_fit_combined.Scale(rh_combined.sumEntries()*10)

h_fit_combined_test = pdf_combined.createHistogram("h_fit_combined_test",obs)
h_fit_combined_test.Scale(rh_combined.sumEntries())
chi2_combined, ndf_combined = getChi2(h_combined, h_fit_combined_test)
chi2_combined = chi2_combined/(ndf_combined-fit_combined_res.floatParsFinal().getSize())

h_combined_plot = h_combined.Clone("h_combined_plot")
h_combined_plot.Rebin(2)
h_combined_plot.GetXaxis().SetTitle(args.mass_obs+" (GeV)")
h_combined_plot.GetYaxis().SetTitle("Events")
h_combined_plot.SetMarkerColor(ROOT.kBlack)
h_combined_plot.SetLineColor(ROOT.kBlack)
h_combined_plot.SetMarkerSize(0.6)
h_combined_plot.SetMarkerStyle(20)
h_combined_plot.GetYaxis().SetRangeUser(0.,h_combined_plot.GetMaximum()*1.25)

combined_mean1.setConstant(True)
combined_mean2.setConstant(True)
combined_sigma1.setConstant(True)
combined_sigma2.setConstant(True)
combined_gauss_frac.setConstant(True)
...
## Prima gaussiana
zjet_mean1 = ROOT.RooRealVar("zjet_mean1","zjet_mean1",91.18,80,110)
zjet_sigma1 = ROOT.RooRealVar("zjet_sigma1","zjet_sigma1",5.0,1,20)
pdf_zjet_gauss1 = ROOT.RooGaussian("pdf_zjet_gauss1","pdf_zjet_gauss1",obs,zjet_mean1,zjet_sigma1)

## Seconda gaussiana
zjet_mean2 = ROOT.RooRealVar("zjet_mean2","zjet_mean2",91.18,80,110)
zjet_sigma2 = ROOT.RooRealVar("zjet_sigma2","zjet_sigma2",15.0,5,50)
pdf_zjet_gauss2 = ROOT.RooGaussian("pdf_zjet_gauss2","pdf_zjet_gauss2",obs,zjet_mean2,zjet_sigma2)

zjet_gauss_frac = ROOT.RooRealVar("zjet_gauss_frac","zjet_gauss_frac",0.7,0.,1.)

pdf_zjet_double_gauss = ROOT.RooAddPdf("pdf_zjet_double_gauss","pdf_zjet_double_gauss",ROOT.RooArgList(pdf_zjet_gauss1,pdf_zjet_gauss2),ROOT.RooArgList(zjet_gauss_frac),True)
...
# Aggiorna le medie delle gaussiane con le correzioni JMS
zjet_mean1_corr = ROOT.RooFormulaVar("zjet_mean1_corr","","@0*(1+@1*@2)",ROOT.RooArgList(zjet_mean1,zjet_jms_unc,CMS_jms_unc))
zjet_mean2_corr = ROOT.RooFormulaVar("zjet_mean2_corr","","@0*(1+@1*@2)",ROOT.RooArgList(zjet_mean2,zjet_jms_unc,CMS_jms_unc))

# Aggiorna le sigma delle gaussiane con le correzioni JMR
zjet_sigma1_corr = ROOT.RooFormulaVar("zjet_sigma1_corr","","@0*(1+@1*@2)",ROOT.RooArgList(zjet_sigma1,zjet_jmr_unc,CMS_jmr_unc))
zjet_sigma2_corr = ROOT.RooFormulaVar("zjet_sigma2_corr","","@0*(1+@1*@2)",ROOT.RooArgList(zjet_sigma2,zjet_jmr_unc,CMS_jmr_unc))

# Ricrea le gaussiane con i parametri corretti
pdf_zjet_gauss1_corr = ROOT.RooGaussian("pdf_zjet_gauss1_corr","pdf_zjet_gauss1_corr",obs,zjet_mean1_corr,zjet_sigma1_corr)
pdf_zjet_gauss2_corr = ROOT.RooGaussian("pdf_zjet_gauss2_corr","pdf_zjet_gauss2_corr",obs,zjet_mean2_corr,zjet_sigma2_corr)

# Modello del segnale come somma delle due gaussiane
pdf_zjet_sig = ROOT.RooAddPdf("pdf_zjet_sig","pdf_zjet_sig",ROOT.RooArgList(pdf_zjet_gauss1_corr,pdf_zjet_gauss2_corr),ROOT.RooArgList(zjet_gauss_frac),True)

# Parametri legacy per compatibilità
zjet_gpeak = ROOT.RooFormulaVar("zjet_gpeak","","@0*(1+@1*(@0+@3)*@2)",ROOT.RooArgList(zjet_gmean,zjet_jms_unc,CMS_jms_unc,zjet_mean1))
...
# Freeze parametri del nuovo modello a doppia gaussiana
zjet_mean1.setConstant(True)
zjet_mean2.setConstant(True)
zjet_sigma1.setConstant(True)
zjet_sigma2.setConstant(True)
zjet_gauss_frac.setConstant(True)
...
# Import del modello combinato
pdf_combined_norm = ROOT.RooRealVar(pdf_combined.GetName()+"_norm","",rh_combined.sumEntries())
pdf_combined_norm.setConstant(True)
w.Import(pdf_combined)
w.Import(pdf_combined_norm)
...
#######################################
## Plot del fit combinato            ##
#######################################

h_combined_plot.Draw("EP")
h_fit_combined_rebin = h_fit_combined.Clone("h_fit_combined_rebin")
h_fit_combined_rebin.Scale(2)  # Scale per il rebin factor
h_fit_combined_rebin.SetLineColor(ROOT.kRed)
h_fit_combined_rebin.SetLineWidth(2)
h_fit_combined_rebin.Draw("hist same")
h_combined_plot.Draw("EPsame")
ROOT.CMS_lumi(c,"%.1f"%(lumi_dict[args.year]),False,False,True)
label.DrawLatex(0.15,0.8,"Combined Fit (ZJet+WJet+GJet)")
label.DrawLatex(0.15,0.75,"#chi^{2}/ndf=%.2f"%(chi2_combined))
label.DrawLatex(0.15,0.7,"Mean1=%.1f, #sigma1=%.1f"%(combined_mean1.getVal(),combined_sigma1.getVal()))
label.DrawLatex(0.15,0.65,"Mean2=%.1f, #sigma2=%.1f"%(combined_mean2.getVal(),combined_sigma2.getVal()))
label.DrawLatex(0.15,0.6,"Frac=%.3f"%(combined_gauss_frac.getVal()))
c.SaveAs(args.output_directory+"/combined_fit_"+args.cat_id+".png","png")
c.SaveAs(args.output_directory+"/combined_fit_"+args.cat_id+".pdf","pdf")
...
## Prima gaussiana per WJet
wjet_mean1 = ROOT.RooRealVar("wjet_mean1","wjet_mean1",80.37,70,95)
wjet_sigma1 = ROOT.RooRealVar("wjet_sigma1","wjet_sigma1",5.0,1,20)
pdf_wjet_gauss1 = ROOT.RooGaussian("pdf_wjet_gauss1","pdf_wjet_gauss1",obs,wjet_mean1,wjet_sigma1)

## Seconda gaussiana per WJet
wjet_mean2 = ROOT.RooRealVar("wjet_mean2","wjet_mean2",80.37,70,95)
wjet_sigma2 = ROOT.RooRealVar("wjet_sigma2","wjet_sigma2",15.0,5,50)
pdf_wjet_gauss2 = ROOT.RooGaussian("pdf_wjet_gauss2","pdf_wjet_gauss2",obs,wjet_mean2,wjet_sigma2)

wjet_gauss_frac = ROOT.RooRealVar("wjet_gauss_frac","wjet_gauss_frac",0.7,0.,1.)
...
# Aggiorna le medie delle gaussiane con le correzioni JMS
wjet_mean1_corr = ROOT.RooFormulaVar("wjet_mean1_corr","","@0*(1+@1*@2)",ROOT.RooArgList(wjet_mean1,wjet_jms_unc,CMS_jms_unc))
wjet_mean2_corr = ROOT.RooFormulaVar("wjet_mean2_corr","","@0*(1+@1*@2)",ROOT.RooArgList(wjet_mean2,wjet_jms_unc,CMS_jms_unc))

# Aggiorna le sigma delle gaussiane con le correzioni JMR
wjet_sigma1_corr = ROOT.RooFormulaVar("wjet_sigma1_corr","","@0*(1+@1*@2)",ROOT.RooArgList(wjet_sigma1,wjet_jmr_unc,CMS_jmr_unc))
wjet_sigma2_corr = ROOT.RooFormulaVar("wjet_sigma2_corr","","@0*(1+@1*@2)",ROOT.RooArgList(wjet_sigma2,wjet_jmr_unc,CMS_jmr_unc))

pdf_wjet_gauss1_corr = ROOT.RooGaussian("pdf_wjet_gauss1_corr","pdf_wjet_gauss1_corr",obs,wjet_mean1_corr,wjet_sigma1_corr)
pdf_wjet_gauss2_corr = ROOT.RooGaussian("pdf_wjet_gauss2_corr","pdf_wjet_gauss2_corr",obs,wjet_mean2_corr,wjet_sigma2_corr)

pdf_wjet = ROOT.RooAddPdf("pdf_wjet","pdf_wjet",ROOT.RooArgList(pdf_wjet_gauss1_corr,pdf_wjet_gauss2_corr),ROOT.RooArgList(wjet_gauss_frac),True)
...
# Freeze parametri del nuovo modello a doppia gaussiana per WJet
wjet_mean1.setConstant(True)
wjet_mean2.setConstant(True)
wjet_sigma1.setConstant(True)
wjet_sigma2.setConstant(True)
wjet_gauss_frac.setConstant(True)
...
# Crea anche la datacard combinata
# sys_combined = {"NLOEWK_corr_combined": (safe_corr(h_zjet_ewk, h_zjet) + safe_corr(h_wjet_ewk, h_wjet))/2, "NLOEWK_corr_zg": safe_corr(h_zg_ewk, h_zg)}
# dc_combined = createCardTemplateCombined(args.cat_id,"workspace_"+args.cat_id+".root",sys_combined,"w")

# print(f"Datacard standard creata: {dc}")
# print(f"Datacard combinata creata: {dc_combined}")
